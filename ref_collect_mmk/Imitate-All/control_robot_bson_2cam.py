        ...
        while episode_index < num_episodes:
            ...
            while timestamp < episode_time_s:
                ...
                for cam_id, cap in caps.items():
                    ...
                    save_path = save_root / f"camera_{cam_id}" / f"frame_{frame_index:06d}.jpg"
                    savers[cam_id].save(save_path, frame)

                elapsed = time.perf_counter() - start_loop_t
                # print(f"Frame {frame_index} done in {elapsed*1000:.2f} ms")
                ...
                # for k in image_keys:
                #     img = observation[k]["data"]
                #     print(f"🔍 Key: {k}, Shape: {img.shape}, dtype: {img.dtype}")
                #     if img.shape[0] <= 2 or img.shape[1] <= 2:
                #         print(f"⚠️ WARNING: Image from {k} is too small ({img.shape})")
