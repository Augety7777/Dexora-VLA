...
class AIRBOTMMK2(AIRBOTMMK2_BASE):
    ...
    def _init_xhand(self):
        """初始化并验证灵巧手连接"""
        # 获取左手信息
        resp_ht = self.xhand_node.get_hand_type("hand_a")
        if resp_ht and resp_ht["code"] == 0:
            self.hand_a_type = resp_ht['data']
            logger.info(f"XHand-A type: {self.hand_a_type}")
        else:
            logger.error("Failed to get XHand-A type")
        
        # 获取SN码
        resp_sn = self.xhand_node.get_serial_number("hand_a")
        if resp_sn and resp_sn["code"] == 0:
            self.hand_a_sn = resp_sn['data']
            logger.info(f"XHand-A SN: {self.hand_a_sn}")
    ...
    # wzr_code
    # def _get_xhand_joint_state(
    #     self, ns: str, comp: str, stamp: Time, pos=None
    # ) -> dict:
    #     data = {}
    #     handle = "joint_state"
    #     data[f"/{ns}/{comp}/{handle}"] = {
    ...
    #     print("Available joint names:", self.joint_names)
    #     print("All_joints:", all_joints)
    #     for comp in self.components:
    #         print(comp)
    #         print(self.joint_names[comp])
    ...
    #                 data[f"action/{comp_eef}/joint_position"] = [0.0]
    #             elif comp in MMK2ComponentsGroup.HEAD_SPINE:
    #                 jq = list(
