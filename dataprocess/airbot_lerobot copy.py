...
class AirbotLeRobotProcessor:
    ...
    def extract_robot_joint_data(self, robot_data: Dict[str, Any]) -> Dict[str, np.ndarray]:
        ...
        try:
            if 'frames' in robot_data:
                ...
                for frame_data in robot_data['frames']:
                    if 'manipulator' in frame_data:
                        manipulator_data = frame_data['manipulator']
                        
                        # 提取左臂数据
                        if 'left_arm' in manipulator_data:
                            left_positions = manipulator_data['left_arm'].get('joint_positions', [0]*6)
                            left_arm_data.append(left_positions[:6])  # 确保6维
                        else:
                            left_arm_data.append([0]*6)
                        
                        # 提取右臂数据
                        if 'right_arm' in manipulator_data:
                            right_positions = manipulator_data['right_arm'].get('joint_positions', [0]*6)
                            right_arm_data.append(right_positions[:6])  # 确保6维
                        else:
                            right_arm_data.append([0]*6)
                
                joint_data['left_arm_joints'] = np.array(left_arm_data)
                joint_data['right_arm_joints'] = np.array(right_arm_data)
                
                self.logger.info(f"机械臂数据: 左臂 {joint_data['left_arm_joints'].shape}, 右臂 {joint_data['right_arm_joints'].shape}")
    ...
    def extract_hand_data(self, hand_data: Dict[str, Any]) -> Dict[str, np.ndarray]:
        ...
        try:
            if 'frames' in hand_data:
                # 初始化数据列表
                left_hand_data = []
                right_hand_data = []
                
                for frame_data in hand_data['frames']:
                    if 'dexterous_hand' in frame_data:
                        hand_frame_data = frame_data['dexterous_hand']
                        
                        # 提取左手数据
                        if 'left_hand' in hand_frame_data:
                            left_positions = hand_frame_data['left_hand'].get('joint_positions', [0]*12)
                            left_hand_data.append(left_positions[:12])  # 确保12维
                        else:
                            left_hand_data.append([0]*12)
                        
                        # 提取右手数据
                        if 'right_hand' in hand_frame_data:
                            right_positions = hand_frame_data['right_hand'].get('joint_positions', [0]*12)
                            right_hand_data.append(right_positions[:12])  # 确保12维
                        else:
                            right_hand_data.append([0]*12)
                
                hand_joint_data['left_hand_joints'] = np.array(left_hand_data)
                hand_joint_data['right_hand_joints'] = np.array(right_hand_data)
                
                self.logger.info(f"灵巧手数据: 左手 {hand_joint_data['left_hand_joints'].shape}, 右手 {hand_joint_data['right_hand_joints'].shape}")
    ...
    def convert_frame_to_lerobot_format(self, 
        ...
        # 构建状态向量 (36维: 6+6+12+12)
        left_arm = robot_joint_data.get('left_arm_joints', np.zeros((1, 6)))[frame_idx] if frame_idx < len(robot_joint_data.get('left_arm_joints', [])) else np.zeros(6)
        right_arm = robot_joint_data.get('right_arm_joints', np.zeros((1, 6)))[frame_idx] if frame_idx < len(robot_joint_data.get('right_arm_joints', [])) else np.zeros(6)
        left_hand = hand_joint_data.get('left_hand_joints', np.zeros((1, 12)))[frame_idx] if frame_idx < len(hand_joint_data.get('left_hand_joints', [])) else np.zeros(12)
        right_hand = hand_joint_data.get('right_hand_joints', np.zeros((1, 12)))[frame_idx] if frame_idx < len(hand_joint_data.get('right_hand_joints', [])) else np.zeros(12)
        
        # 组合状态
        state = np.concatenate([left_arm, right_arm, left_hand, right_hand])
        frame["states"] = state.astype(np.float32)
        
        # 动作 (这里假设action = next_state - current_state，你可能需要根据实际情况调整)
        if frame_idx < len(robot_joint_data.get('left_arm_joints', [])) - 1:
            next_left_arm = robot_joint_data.get('left_arm_joints', np.zeros((1, 6)))[frame_idx + 1]
            next_right_arm = robot_joint_data.get('right_arm_joints', np.zeros((1, 6)))[frame_idx + 1]
            next_left_hand = hand_joint_data.get('left_hand_joints', np.zeros((1, 12)))[frame_idx + 1]
            next_right_hand = hand_joint_data.get('right_hand_joints', np.zeros((1, 12)))[frame_idx + 1]
            
            next_state = np.concatenate([next_left_arm, next_right_arm, next_left_hand, next_right_hand])
