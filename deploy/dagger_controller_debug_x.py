...
- Human intervention mode with keyboard control (Hybrid: Arms=Human IK, Hands=AI Policy)
- Automatic data collection for DAgger training
- Smooth mode switching between policy and hybrid human control
...
def setup_keyboard_listener():
    """设置键盘监听器用于模式切换和遥操作控制（方案2：速度控制）"""
    global control_mode, should_exit, active_arm, teleop_velocity
            ...
            if key == keyboard.Key.space:
                ...
                with mode_lock:
                    if control_mode == "policy":
                        ...
                        logging.info("=" * 70)
                        logging.info("🔴 切换到混合控制模式")
                        logging.info("  ✋ 手臂: 人类键盘遥操作（IK控制）")
                        logging.info("  🤖 手部: AI策略自动控制")
    ...
    def on_release(key):
        """松开键时清零对应方向的速度"""
        global teleop_velocity, active_arm
        try:
...
def get_human_action_from_teleop(robot_interface, policy_action=None):
    """
    从键盘遥操作获取人类控制的动作（混合模式）
    
    每个控制周期应用速度增量，更新目标位姿，
    然后通过MMK2 KDL IK求解得到手臂关节角度。
    
    ✅ 关键设计：手臂由人类控制（IK），手部由AI策略控制
    
    Args:
        robot_interface: 机器人接口
        policy_action: AI策略动作（36维），用于提取手部控制
    """
    global teleop_target_pose, teleop_velocity
    ...
    if policy_action is not None:
        # 从policy_action提取手部动作
        left_hand = policy_action[6:18]    # 策略的左手动作
        right_hand = policy_action[24:36]  # 策略的右手动作
        # logging.debug(f"🤖 使用AI策略控制手部")
    else:
        # 降级：使用当前状态
        left_hand = np.array(obs['hand_left']).flatten()
        right_hand = np.array(obs['hand_right']).flatten()
        logging.debug(f"⚠️ 无policy_action，手部保持当前状态")
...
def model_inference_loop(config, robot_interface, policy, vision_encoder, image_processor):
    ...
    logging.info("=" * 70)
    logging.info("🚀 Human-in-the-Loop 混合控制系统")
    logging.info("=" * 70)
    logging.info("📋 模式控制：")
    logging.info("  空格键 - 切换模式")
    logging.info("    🟢 AI全自主模式: 手臂+手部都由AI策略控制")
    logging.info("    🔴 混合控制模式: 手臂=人类键盘遥操作(IK) + 手部=AI策略")
    logging.info("  S键    - 保存干预数据")
    logging.info("  Q键    - 退出程序")
    logging.info("")
    logging.info("🎮 遥操作控制（混合模式下）：")
    logging.info("  手臂选择:")
    logging.info("    1 = 👈 左臂  |  2 = 👉 右臂  |  3 = 👐 双臂")
    logging.info("  手臂位置 (按住移动，松开停止):")
    logging.info("    W/X = 前进/后退  |  A/D = 左移/右移  |  R/F = 上升/下降")
    logging.info("  手臂旋转 (按住旋转，松开停止):")
    logging.info("    I/K = Roll(+/-)  |  J/L = Pitch(+/-)  |  U/O = Yaw(+/-)")
    logging.info("  手部控制:")
    logging.info("    🤖 自动由AI策略控制（基于视觉和任务）")
    logging.info("")
    logging.info(f"📌 默认: 👈 LEFT 手臂")
    logging.info("=" * 70)
    ...
    try:
        while t < max_steps and not should_exit:
            ...
            if current_mode == "policy":
                ...
                # 人类控制模式：获取人类动作（混合控制：手臂=人类IK，手部=AI策略）
            ...
            # Interpolate if needed (方案1：人类模式下禁用插值)
            ...
            if t % 10 == 0:
                ...
                if current_mode == "human":
                    ...
                    status_msg += (
                        f"\n  ✋ 手臂 {arm_symbol} {active_arm.upper()} | "
