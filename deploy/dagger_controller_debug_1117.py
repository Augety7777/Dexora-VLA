...
teleop_step_size = 0.01  # 1cm per keypress
teleop_rot_step = 0.05   # ~3 degrees per keypress
...
def setup_keyboard_listener():
    """设置键盘监听器用于模式切换和遥操作控制"""
    global control_mode, should_exit, active_arm, teleop_target_pose
    ...
    def on_press(key):
        ...
        try:
            ...
            if key == keyboard.Key.space:
                ...
                with mode_lock:
                    if control_mode == "policy":
                        control_mode = "human"
                        logging.info("🔴 切换到人类控制模式 - 使用键盘遥操作")
                        logging.info("提示: 1/2=切换手臂, WASD/RF=位置, IJKL/UO=旋转, ZXCV=手部")
                    else:
                        control_mode = "policy"
            ...
            # ===== 手臂选择 (仅在人类模式下有效) =====
                ...
                if key.char == '1':
                    active_arm = 'left'
                    logging.info(f"👈 激活左臂控制")
                elif key.char == '2':
                    active_arm = 'right'
                    logging.info(f"👉 激活右臂控制")
                elif key.char == '3':
                    active_arm = 'both'
                    logging.info(f"👐 激活双臂控制")
                
                # ===== 位置控制 (WASD + R/F) =====
                elif key.char == 'w':  # +X (前)
                    for arm in get_active_arms():
                        teleop_target_pose[arm]['pos'][0] += teleop_step_size
                    logging.debug(f"➡️ 前移 {teleop_step_size}m")
                elif key.char == 's':  # -X (后) - 注意: s 也用于保存，这里优先保存
                    pass  # s键已用于保存
                elif key.char == 'a':  # +Y (左)
                    for arm in get_active_arms():
                        teleop_target_pose[arm]['pos'][1] += teleop_step_size
                    logging.debug(f"⬅️ 左移 {teleop_step_size}m")
                elif key.char == 'd':  # -Y (右)
                    for arm in get_active_arms():
                        teleop_target_pose[arm]['pos'][1] -= teleop_step_size
                    logging.debug(f"➡️ 右移 {teleop_step_size}m")
                elif key.char == 'r':  # +Z (上)
                    for arm in get_active_arms():
                        teleop_target_pose[arm]['pos'][2] += teleop_step_size
                    logging.debug(f"⬆️ 上移 {teleop_step_size}m")
                elif key.char == 'f':  # -Z (下)
                    for arm in get_active_arms():
                        teleop_target_pose[arm]['pos'][2] -= teleop_step_size
                    logging.debug(f"⬇️ 下移 {teleop_step_size}m")
                
                # ===== 旋转控制 (IJKL + U/O) =====
                elif key.char == 'i':  # +Roll
                    for arm in get_active_arms():
                        teleop_target_pose[arm]['rot'][0] += teleop_rot_step
                    logging.debug(f"🔄 Roll+ {teleop_rot_step}rad")
                    ...
                    for arm in get_active_arms():
                        teleop_target_pose[arm]['rot'][1] += teleop_rot_step
                    logging.debug(f"🔄 Pitch+ {teleop_rot_step}rad")
                    ...
                    for arm in get_active_arms():
                        teleop_target_pose[arm]['rot'][2] += teleop_rot_step
                    logging.debug(f"🔄 Yaw+ {teleop_rot_step}rad")
                elif key.char == 'o':  # -Yaw
                    for arm in get_active_arms():
                        teleop_target_pose[arm]['rot'][2] -= teleop_rot_step
                    logging.debug(f"🔄 Yaw- {teleop_rot_step}rad")
    ...
    logging.info("  1/2/3=选择手臂 | WASD/RF=移动 | IJKL/UO=旋转")
    return listener
...
def get_human_action_from_teleop(robot_interface):
    ...
    使用全局变量 teleop_target_pose 中存储的目标位姿，
    通过MMK2 KDL IK求解得到手臂关节角度，
    手部姿态保持当前状态
    """
    global teleop_target_pose
    
    # 获取当前机器人状态
    obs = get_observations(robot_interface)
    
    # 🔍 调试：打印原始观测维度
    logging.debug(f"🔍 obs维度: arm_left={len(obs['arm_left'])}, arm_right={len(obs['arm_right'])}, "
                  f"hand_left={len(obs['hand_left'])}, hand_right={len(obs['hand_right'])}")
    ...
    print("teleop_target_pose['left']['pos']", teleop_target_pose['left']['pos'])
    print("teleop_target_pose['right']['pos']", teleop_target_pose['right']['pos'])
    # input()
    # 右臂IK求解
    ...
    # input()
    # 手部姿态保持当前状态
    left_hand = np.array(obs['hand_left']).flatten()
    right_hand = np.array(obs['hand_right']).flatten()
    
    # 🔍 调试：打印各部分维度
    logging.debug(f"🔍 组件维度: left_arm={left_arm_joints.shape[0]}, left_hand={left_hand.shape[0]}, "
                  f"right_arm={right_arm_joints.shape[0]}, right_hand={right_hand.shape[0]}")
    ...
    logging.debug(f"✅ human_action shape: {human_action.shape}")
    
    return human_action


def initialize_teleop_from_current_state(robot_interface):
    """从当前机器人状态初始化遥操作目标位姿（使用MMK2 KDL FK）"""
    global teleop_target_pose, mmk2_kdl, MMK2_KDL_AVAILABLE
...
def model_inference_loop(config, robot_interface, policy, vision_encoder, image_processor):
    ...
    logging.info("🔵 进入 model_inference_loop 函数")
    logging.info("🔵 正在重置MMK...")
    robot_interface.reset_mmk()
    logging.info("✅ MMK重置完成")

    
    logging.info("🔵 正在设置键盘监听...")
    listener = setup_keyboard_listener()
    logging.info("✅ 键盘监听器已启动")
    
    logging.info("🔵 正在更新观测窗口...")
    update_observation_window(config, robot_interface)
    logging.info("✅ 观测窗口已更新")
    
    logging.info("🔵 正在初始化遥操作位姿...")
    initialize_teleop_from_current_state(robot_interface)
    logging.info("✅ 遥操作位姿已初始化")
    ...
    logging.info("🚀 启动Human-in-the-Loop推理循环（带手腕IK遥操作）")
    logging.info("=" * 70)
    logging.info("模式控制：")
    logging.info("  空格键 - 切换 策略模式(policy) ↔ 人类控制模式(teleop)")
    logging.info("  S键    - 保存干预数据")
    logging.info("  Q键    - 退出程序")
    logging.info("")
    logging.info("遥操作控制（人类模式下）：")
    logging.info("  1/2/3      - 选择左臂/右臂/双臂")
    logging.info("  W/A/D      - 前进/左移/右移")
    logging.info("  R/F        - 上升/下降")
    logging.info("  I/K        - Roll 旋转（+/-）")
    logging.info("  J/L        - Pitch 旋转（+/-）")
    logging.info("  U/O        - Yaw 旋转（+/-）")
    logging.info("=" * 70)
    ...
    try:
        while t < max_steps and not should_exit:
            ...
            if prev_mode == "policy" and current_mode == "human":
                logging.info("🔄 检测到切换到人类模式，重新初始化遥操作位姿...")
                try:
                    initialize_teleop_from_current_state(robot_interface)
                except Exception as e:
            ...
            if t % 10 == 0:
                ...
                if current_mode == "human":
                    ...
                    status_msg += (
                        f"\n  {arm_symbol} {active_arm} | "
                        f"Pos: [{target['pos'][0]:.3f}, {target['pos'][1]:.3f}, {target['pos'][2]:.3f}] | "
                        f"Rot: [{target['rot'][0]:.2f}, {target['rot'][1]:.2f}, {target['rot'][2]:.2f}]"
                    )
