...
def find_episode_folders(data_folder: str) -> List[str]:
    ...
    支持：
      - 传入 Dex-RDT/data/kefei（直接在其下查找 episode_*）
      - 传入 Dex-RDT/data（在 data/kefei 下查找）
      - 传入具体 action_* 目录（在该目录下查找 episode_*）
      - 传入具体 episode_* 目录（直接检查该目录）
    """
    episode_folders: List[str] = []

    data_folder = os.path.abspath(data_folder)
    base = os.path.basename(os.path.normpath(data_folder))

    # 情况1：直接指定某个 episode_* 目录
    if base.startswith("episode_") and os.path.isdir(data_folder):
        episode_file = os.path.join(data_folder, "episode_0.bson")
        xhand_file = os.path.join(data_folder, "xhand_control_data.bson")
        if os.path.exists(episode_file) and os.path.exists(xhand_file):
            return [data_folder]
        else:
            if os.path.exists(episode_file) and not os.path.exists(xhand_file):
                print(f"警告: {data_folder} 中缺少 xhand_control_data.bson")
            if os.path.exists(xhand_file) and not os.path.exists(episode_file):
                print(f"警告: {data_folder} 中缺少 episode_0.bson")
            return []

    # 情况2：直接指定某个 action_* 目录
    action_dirs: List[str] = []
    if base.startswith("action") and os.path.isdir(data_folder):
        action_dirs = [data_folder]
        kefei_folder = os.path.dirname(data_folder)
    else:
        # 情况3：传入 data/kefei 或 data 或更上层目录
        if base == "kefei":
            kefei_folder = data_folder
        elif os.path.isdir(os.path.join(data_folder, "kefei")):
            kefei_folder = os.path.join(data_folder, "kefei")
        elif os.path.isdir(os.path.join(data_folder, "data", "kefei")):
            kefei_folder = os.path.join(data_folder, "data", "kefei")
        else:
            # 宽松处理：当作已在 kefei 下
            kefei_folder = data_folder

        action_dirs = glob.glob(os.path.join(kefei_folder, "action*"))

    if not os.path.exists(kefei_folder):
        print(f"警告: 未找到 kefei 目录: {kefei_folder}")
        return episode_folders

    # 先收集 kefei 目录下直接的 episode_* 目录（满足“Dex-RDT/data/kefei 即为一整个 action”的场景）
    for episode_folder in glob.glob(os.path.join(kefei_folder, "episode_*")):
    ...
    # 兼容旧结构：action*/episode_*
    for action_folder in action_dirs:
    ...
    # 去重并排序
    return sorted(set(episode_folders))
